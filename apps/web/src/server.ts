...
" sortValues: Array.from(row.cells).map((cell) => (cell.textContent ?? '').replace(/\\s+/g, ' ').trim())",
" }));",
" let query = '';",
" let currentPage = 1;",
...
" const comparison = collator.compare(left.sortValues[sortColumn] ?? '', right.sortValues[sortColumn] ?? '');",
" if (comparison !== 0) {",
