...
expect(legacyContractView.headers.get("location")).toBe("/admin/contracts/contract-1");
 ...
 expect(petDetailHtml).toContain('/client/form_requests_create.php?form_type=pet_form&pet_id=pet-1');
  ...
  it("renders admin content, settings, and operations pages and supports content mutations", async () => {
    ...
    const calendarSettings = await fetch(`${baseUrl}/admin/settings?category=calendar`, {
      headers: {
        cookie: adminCookie ?? ""
      }
    });
    ...
    expect(calendarSettings.status).toBe(200);
    ...
    const calendarSettingsHtml = await calendarSettings.text();
...
expect(settingsHtml).toContain("settings-summary-grid");
    ...
    expect(calendarSettingsHtml).toContain("Google Calendar Connection");
    expect(calendarSettingsHtml).toContain("Legacy Calendar Screen");
  ...
  it("renders admin appointment type, form template, email template, and scheduled task management pages with legacy aliases", async () => {
    ...
    expect(appointmentTypeDetailHtml).toContain("Booking Slug");
    expect(appointmentTypeDetailHtml).toContain("/backend/public/book.php?link=private-coaching-link");
    expect(appointmentTypeDetailHtml).toContain("Open Public Booking Page");
