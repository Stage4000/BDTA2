import { buildProductionJobRuntime, createDefaultJobHandlers } from "../apps/jobs/src/bootstrap.js";
...
describe("job worker bootstrap", () => {
  ...
  it("processes workflow enrollments with production handlers", async () => {
    class WorkflowProcessorExecutor extends RecordingExecutor {
      override async execute<T>(sql: string, params: unknown[] = []): Promise<[T, { affectedRows?: number }]> {
        ...
        if (sql.includes("FROM workflow_enrollments we")) {
          return [[{
        ...
        if (sql.includes("FROM email_outbox")) {
          return [[{
            ...
            html_body: "<p>Workflow follow-up</p>",
            template_key: "workflow_notification"
          }] as T, { affectedRows: 0 }];
        }
